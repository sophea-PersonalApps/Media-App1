...
scanner = scanner.replace('CameraSectionMode.PHOTO, CameraSectionMode.VIDEO -> context.startActivity(Intent(context, MainActivity::class.java).putExtras(cameraModeIntent(mode)))','CameraSectionMode.PHOTO, CameraSectionMode.VIDEO -> { context.startActivity(Intent(context, MainActivity::class.java).putExtras(cameraModeIntent(mode))); (context as? ComponentActivity)?.overridePendingTransition(R.anim.slide_in_left, R.anim.slide_out_right) }')
scanner = scanner.replace('CameraSectionMode.QR -> context.startActivity(Intent(context, QrScannerActivity::class.java))','CameraSectionMode.QR -> { context.startActivity(Intent(context, QrScannerActivity::class.java)); (context as? ComponentActivity)?.overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left) }')
...
checks = {
    ...
    "gallery stable viewer": '.pointerInput(uri)' in gallery_final and 'detectTransformGestures' in gallery_final and 'MediaViewer(uri: Uri, isVideo: Boolean, onBack: () -> Unit, onShare: () -> Unit, onEdit: (() -> Unit)?, onDelete: () -> Unit)' in gallery_final,
    ...
    "gallery no swipe state": 'swipeOffset' not in gallery_final and 'videoNavigationStarted' not in gallery_final and 'detectHorizontalDragGestures' not in gallery_final,
    "gallery no navigation callback": 'onNavigate(' not in gallery_final,
    "gallery no current-index viewer state": 'currentIndex' not in gallery_final and 'selectedIndex' not in gallery_final,
