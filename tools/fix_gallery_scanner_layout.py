...
# Scanner capture controls: MORE | shutter | PAGES. There is deliberately no
# second capture button and no flip control. Pages opens the existing preview.
...
if start < 0 or end < 0:
    raise SystemExit("Scanner capture control block not found")
end = scanner.find('                }', end)
if end < 0:
    raise SystemExit("Scanner shared control block closing brace not found")
end += len('                }')
scanner_controls = '''                ScannerSectionControls(
                    pages = pages,
                    onCapture = onCapture,
                    onPages = onFinish,
                    onModeSelected = { scannerModeAction(context, it) }
                )'''
scanner = scanner[:start] + scanner_controls + scanner[end:]

# Remove the remaining top-left Back button from the scanner preview screen.
scanner = scanner.replace('            IconButton(onClick = onBack) { Icon(Icons.Default.ArrowBack, "Back to scanner", tint = ComposeColor.White) }\n', '')
# Scanner -> QR uses the same forward slide direction as Camera -> Video.
...
# Gallery continuous interactive swipe. The adjacent images are preloaded, the
# current image and adjacent image move together with the finger, and release
# either completes a full-screen slide or springs back. Navigation only happens
# after the animation reaches the edge, so the next image never gets stuck at 50%.
...
gallery = replace_once(GALLERY, gallery, old_gesture, new_gesture, "gallery continuous swipe gesture")
gallery = replace_once(GALLERY, gallery,
    ...
    '                            if (photoZoom <= 1f && swipeOffset != 0f) {\n                                val adjacentBitmap = if (swipeOffset < 0f) nextBitmap else previousBitmap\n                                adjacentBitmap?.let { adjacent ->\n                                    Image(adjacent.asImageBitmap(), "Adjacent photo", Modifier.fillMaxSize().padding(8.dp).graphicsLayer { translationX = swipeOffset + if (swipeOffset < 0f) viewportWidth.toFloat() else -viewportWidth.toFloat() }, contentScale = ContentScale.Fit)\n                                }\n                            }', "gallery continuous adjacent render")
GALLERY.write_text(gallery, encoding="utf-8")

main = MAIN.read_text(encoding="utf-8")
main = replace_once(MAIN, main, 'CameraSectionMode.SCAN -> { startActivity(Intent(this, ScannerActivity::class.java)); overridePendingTransition(0, 0) }', 'CameraSectionMode.SCAN -> { startActivity(Intent(this, ScannerActivity::class.java)); overridePendingTransition(android.R.anim.slide_in_right, android.R.anim.slide_out_left) }', "video to scanner animation")
MAIN.write_text(main, encoding="utf-8")

qr = QR.read_text(encoding="utf-8")
qr = replace_once(QR, qr, '                IconButton(onClick = onBack, modifier = Modifier.align(Alignment.TopStart).statusBarsPadding().padding(8.dp)) { Icon(Icons.Default.ArrowBack, "Back", tint = Color.White) }\n', '', "QR top-left back button")
qr = replace_once(QR, qr, 'onOpenScanner = { startActivity(Intent(this, ScannerActivity::class.java)); overridePendingTransition(0, 0) }', 'onOpenScanner = { startActivity(Intent(this, ScannerActivity::class.java)); overridePendingTransition(android.R.anim.slide_in_left, android.R.anim.slide_out_right) }', "QR to scanner animation")
QR.write_text(qr, encoding="utf-8")

checks = {
    "scanner bottom anchored": '.align(Alignment.BottomCenter)' in scanner,
    "scanner custom controls": 'ScannerSectionControls(' in scanner and 'Text("PAGES ${pages.size}"' in scanner,
